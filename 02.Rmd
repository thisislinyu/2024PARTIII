...
98.6% of the subjects has complete information. 
...
missing_summary <- miss_dat %>%
  summarise_all(~ sum(is.na(.))) %>%
  tidyr::gather(key = "variable", value = "missing_count") %>%
  mutate(missing_percentage = missing_count / nrow(data) * 100)


miss_plot <- ggplot(missing_summary, aes(x = reorder(variable, -missing_count), y = missing_count)) +
  geom_bar(stat = "identity", fill = "lightblue",size=0.1) +
  geom_text(aes(label = paste0(missing_count, " (", round(missing_percentage, 2), "%)")), 
